...
void main() {
   ...
   if (uUseCubeMapping) {
	 reflectionColor = texture(uTextureCubeMap,reflectionVector).rgb; 
   } else {
	reflectionVector.z += 1;
	reflectionVector = normalize(reflectionVector);
	vec2 sphereCoord = reflectionVector.st;
	sphereCoord *= 0.5;
	sphereCoord += vec2(0.5);
	reflectionColor = texture(uTextureSphereMap, sphereCoord).rgb;
   }
